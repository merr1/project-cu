...
export class TeachEuComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.http
      .get<any>('https://www.eupoltrans.cu.edu.ge:8000/teach-course/', {
        headers: headers,
      })
      .subscribe((data) => {
        this.data = data;
    ...
    this.http
      .get<any>('https://www.eupoltrans.cu.edu.ge:8000/teach-mooc/', {
        headers: headers,
      })
      .subscribe((data) => {
        this.dataVideo = data;
        for (let i = 0; i < data.length; i++) {
          this.dataVideo[i].url = this.sanitizer.bypassSecurityTrustResourceUrl(
